...
6. Keep all changes reviewable as separate pull requests and do not merge directly into `main`.

## Pull request implementation plan

### PR 1: Visual analysis and redesign rationale

Adds this design analysis document so the owner can review the design rationale before code changes are accepted.

### PR 2: Formal government visual theme layer

Adds a site-wide government-style theme stylesheet and a small bootstrap hook so the visual treatment can be reviewed without replacing the existing core stylesheet. This should adjust colors, contrast, masthead surfaces, buttons, cards, notices, and footer treatment.

### PR 3: Government-style homepage structure

Updates the homepage information architecture to use a task-oriented public-service layout, an official notice area, a service directory pattern, and clearer support pathways. This should be submitted separately so the owner can evaluate structural changes independently from palette changes.
